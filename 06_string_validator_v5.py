"""Combined 06_string_validator_v4 and 07_string_analyser_v3 together.
Made the split_order component into its own function "split_order(choice)"
To keep it generic I had to remove the ‘question’ parameter from the
get_choice() function and replaced it with choice so that it can be used for
snacks and the yes/no question.
Also to include quantity of snacks, the valid_snacks list was changed from
1, 2 etc. to (1, (2, (3 – to ensure a number was not the first and only item
in the string being evaluated.
Also added a maximum of 4 for the number of any particular snack in one order
"""
import re
